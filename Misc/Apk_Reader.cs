...
namespace Menu_Creator_By_Francois284Modz
{
    public class Apk_Reader
    {
        ...
        {
            string exePath = Path.Combine(Directory.GetCurrentDirectory(), @"tools\" + fileName);
            if (File.Exists(exePath))
            {
                exePath = Path.Combine(Directory.GetCurrentDirectory(), @"tools\" + fileName);

            }
            if (!File.Exists(exePath))
            {
                File.WriteAllBytes(exePath, Properties.Resources.aapt);
            }
